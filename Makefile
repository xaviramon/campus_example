...
create-group_vars: ## Generate groupvar files based on the EXOFFICE1 template (Example Office1). Execution: make create-group_vars OFFICE=<Name of the office> THEATER=<Region>
	...
	cd group_vars/$(OFFICE); rename -c "^(ENDPOINTS_)(EXOFFICE1)([\w]*)(.yml)$\" "\(1)$(OFFICE)\(2)\(3)"; sed 's/EXOFFICE1/$(OFFICE)/g' fabric.yml > tmp; sed 's/THEATER/$(THEATER)/g' tmp > tmp2 ; rm fabric.yml; cat tmp2 > fabric.yml; rm tmp tmp2
